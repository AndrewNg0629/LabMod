...
public class LabMod implements ModInitializer {
	...
	public static final ModConfig TEST_CONFIG = new ModConfig(
			...
			false,
			true,
			new ModConfig.ModIdValidationConfig(
					false,
					List.of(),
					List.of()
			),
			new ModConfig.ToggleTweaksSection1(
					false,
					false,
					false,
					false,
			...
			new ModConfig.ToggleTweaksSection2(
					false,
					false,
					false
			),
			new ModConfig.ValueTweaks(
					new ModConfig.ValueTweaks.FireballAutoDiscarding(
							false,
							200
					),
					new ModConfig.ValueTweaks.StuffedShulkerBoxStacking(
							false,
							1
					),
					new ModConfig.ValueTweaks.ShulkerBoxNesting(
							true,
							2
					),
					new ModConfig.ValueTweaks.WardenAttributesControl(
							false,
							500.0,
							1.0,
							30.0,
					...
					new ModConfig.ValueTweaks.WardenSonicBoomControl(
							false,
							true,
							10.0,
							1.0,
					...
					new ModConfig.ValueTweaks.PlayerDeathItemProtection(
							false,
							12000,
							false,
							false
			...
			new ModConfig.ItemEditorConfig(
					false,
					List.of()
			)
	);
	...
	public void onInitialize() {
		...
		//JsonElement result = ModConfig.CODEC.encodeStart(JsonOps.INSTANCE, TEST_CONFIG).result().orElseThrow();
